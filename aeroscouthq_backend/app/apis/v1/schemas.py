...
class PoiSearchResponse(BaseModel):
    """Schema for POI Search Response"""
    success: bool
    data: dict # Will return the raw dictionary from Trip.com API directly
...
class FlightSearchRequest(BaseModel):
    ...
    preferred_currency: Optional[str] = Field("CNY", description="Preferred currency for prices (e.g., 'USD', 'EUR')", json_schema_extra={"example": "CNY"})
    preferred_locale: Optional[str] = Field(None, description="Preferred locale for results (e.g., 'en', 'zh')", json_schema_extra={"example": "en"})
    market: Optional[str] = Field("cn", description="Market code for the search (e.g., 'us', 'cn')", json_schema_extra={"example": "cn"}) # Added market based on _build_kiwi_variables usage
